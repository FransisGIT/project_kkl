jQuery.index = {
    ...
    init: function () {
        ...
        self.data.searchInput = $('#search-partnership');
    ...
    initTable: function () {
        ...
        self.data.table = $('#ohmytable').DataTable({
            ...
            serverSide: true,
            ajax: {
                url: '/admin/pages/partnership/request',
                type: 'POST',
                headers: {
                        ...
                        if (data.logo) {
                            return `
                                <div class='d-flex justify-content-center align-items-center'>
                                    <img src='${BASE_URL}/uploads/partnership/${data.logo}' alt='logo' class='rounded' width='80' height='80' style="object-fit: cover;"/>
                                </div>
                            `;
                        } else {
                            return `
                        ...
                        return `
                            <div class="d-flex justify-content-center gap-2">
                                ...
                                <button class="btn btn-sm btn-danger btn-hapus"
                                    data-id="${data.id}" data-name="${data.company}" title="Hapus">
    ...
    setEvents: function () {
        ...
        $("#ohmytable").on('click', "button.btn-hapus", function () {
            ...
            $.confirm({
                ...
                type: 'orange',
                columnClass: 'small',
                content: 'Apakah Anda yakin ingin menghapus mitra <strong>' + name + '</strong>? Data yang dihapus tidak dapat dikembalikan.',
                buttons: {
                    cancel: {
                        text: 'Tidak',
                        btnClass: 'btn-red',
        ...
        $(document).on('click', '.btn-edit', function () {
            ...
            $('#tag-form').attr('action', '/admin/pages/partnership/update');

            if ($('#partnership_id').length === 0) {
                $('#tag-form').append('<input type="hidden" name="id" id="partnership_id" value="' + row.id + '">');
            } else {
                $('#partnership_id').val(row.id);
            }
        ...
        $('#btn-cancel').on('click', function () {
            $('#partnership_id').remove();
            $('#company').val('');
            $('#status').val(null).trigger('change');

            $('#tag-form').attr('action', '/admin/pages/partnership/store');
